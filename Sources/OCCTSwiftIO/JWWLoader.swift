...
enum JWWLoader {
    ...
    static func load(from url: URL) throws -> ShapeLoadResult {
        ...
        func emit(_ entity: JWW.Entity) {
            ...
            case .arc(let c, let r, let start, let sweep, let tilt, let ratio, let full, _, _):
                let cx = c.x
                let cy = c.y
                ...
                if abs(ratio - 1) < 1e-9 {  // circle / circular arc
                    ...
                    if full || abs(abs(sweep) - 2 * .pi) < 1e-6 {
                        ...
                        let a = tilt + start
                        let b = tilt + start + sweep
                        (p1, p2) = (min(a, b), max(a, b))
                    }
                    if let s = Shape.edgeFromCircle(
                        center: SIMD3(cx, cy, 0), axis: axis, radius: r, p1: p1, p2: p2)
                    {
                        shapes.append(s)
                    }
                } else {  // ellipse → polyline (carries tilt)
                    emitEllipsePolyline(
                        cx: cx, cy: cy, r: r, ratio: ratio, tilt: tilt,
                        start: start, sweep: sweep, full: full, into: &shapes)
                }
            case .point(let pt, _, _):
    ...
    private static func emitEllipsePolyline(
        cx: Double, cy: Double, r: Double, ratio: Double, tilt: Double,
        start: Double, sweep: Double, full: Bool, into shapes: inout [Shape]
    ) {
        let n = 48
        let a0 = full ? 0 : start
        let a1 = full ? 2 * .pi : start + sweep
        let ct = cos(tilt)
        let st = sin(tilt)
        func pt(_ a: Double) -> SIMD3<Double> {
            let ex = r * cos(a)
            let ey = r * ratio * sin(a)
