...
/// Native STL reader/writer (ASCII + binary).
///
/// No OCCT.
public enum STL {
    ...
    static func readASCII(_ data: Data) -> [SIMD3<Float>] {
        ...
        data.withUnsafeBytes { (raw: UnsafeRawBufferPointer) in
            ...
            while i < n {
                ...
                var ok = true
                for k in 0..<6 where b[s + k] != kw[k] { ok = false }
                ...
                while p < e, c.count < 3 {
                    ...
                    let t = p
                    while p < e, b[p] != 32, b[p] != 9, b[p] != 13 { p += 1 }
                    if p > t,
                        let f = Float(
                            String(decoding: UnsafeBufferPointer(rebasing: b[t..<p]), as: UTF8.self)
                        )
                    {
                        c.append(f)
                    }
    ...
    public static func binaryData(_ mesh: Mesh) -> Data {
        ...
        var nn = UInt32(mesh.triangleCount)
        withUnsafeBytes(of: &nn) { data.append(contentsOf: $0) }
        func f(_ v: Float) {
            var x = v
