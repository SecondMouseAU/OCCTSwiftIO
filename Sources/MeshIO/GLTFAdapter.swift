...
extension MeshIO {
    ...
    /// Read **glTF** / **GLB** into a ``Mesh``.
    ///
    /// SwiftGLTF handles the hard decoding (JSON/GLB container, buffers, external `.bin`,
    /// accessors); we walk the scene graph, decode POSITION + index accessors ourselves (avoiding
    /// SwiftGLTF's RealityKit-importing convenience helpers), and bake node transforms.
    static func readGLTF(url: URL, weldEpsilon: Float) throws -> Mesh {
        ...
        func indices(_ acc: SwiftGLTF.Accessor) throws -> [Int] {
            ...
            d.withUnsafeBytes { raw in
                for i in 0..<acc.count {
                    ...
                    case .UNSIGNED_BYTE:
                        out.append(Int(raw.loadUnaligned(fromByteOffset: i, as: UInt8.self)))
                    case .UNSIGNED_SHORT:
                        out.append(Int(raw.loadUnaligned(fromByteOffset: i * 2, as: UInt16.self)))
                    case .UNSIGNED_INT:
                        out.append(Int(raw.loadUnaligned(fromByteOffset: i * 4, as: UInt32.self)))
                    default: break
                    }
                }
            }
        ...
        func local(_ n: SwiftGLTF.Node) -> simd_float4x4 {
            // matrix XOR TRS per spec
            if let m = n.matrix, m != matrix_identity_float4x4 { return m }
            var t = matrix_identity_float4x4
            if let s = n.scale { t = simd_float4x4(diagonal: SIMD4(s, 1)) }
            if let r = n.rotation {
                t = simd_float4x4(simd_quatf(ix: r.x, iy: r.y, iz: r.z, r: r.w)) * t
            }
            if let tr = n.translation {
                var tm = matrix_identity_float4x4
                tm.columns.3 = SIMD4(tr, 1)
                t = tm * t
            }
            return t
        }
        func emit(_ prim: SwiftGLTF.Mesh.Primitive, _ world: simd_float4x4) throws {
            ...
            let wp = pos.map { p -> SIMD3<Float> in
                let v = world * SIMD4(p, 1)
                return SIMD3(v.x, v.y, v.z)
            }
            let idx =
                try prim.indices.map { try indices($0.resolve(in: doc)) } ?? Array(0..<wp.count)
            var t = 0
            while t + 2 < idx.count {
                let a = idx[t]
                let b = idx[t + 1]
                let c = idx[t + 2]
                t += 3
                if a < wp.count, b < wp.count, c < wp.count {
                    soup.append(wp[a])
                    soup.append(wp[b])
                    soup.append(wp[c])
                }
        ...
        if soup.isEmpty {  // no scene graph → meshes untransformed
            for mesh in doc.meshes {
                for prim in mesh.primitives { try emit(prim, matrix_identity_float4x4) }
            }
        }
    ...
    private static func gltfBin(_ mesh: Mesh) -> (
        bin: Data, posLen: Int, idxLen: Int, lo: SIMD3<Float>, hi: SIMD3<Float>
    ) {
        var bin = [UInt8]()
        func f32(_ v: Float) {
    ...
    private static func gltfManifest(
        _ mesh: Mesh,
        _ b: (bin: Data, posLen: Int, idxLen: Int, lo: SIMD3<Float>, hi: SIMD3<Float>),
        bufferURI: String?
    ) -> String {
        func n(_ v: Float) -> String { String(v) }
        let buffer =
            bufferURI.map { "{\"uri\":\"\($0)\",\"byteLength\":\(b.bin.count)}" }
            ?? "{\"byteLength\":\(b.bin.count)}"
    ...
    static func writeGLTF(_ mesh: Mesh) -> Data {
        let b = gltfBin(mesh)
        let json = gltfManifest(
            mesh, b,
            bufferURI: "data:application/octet-stream;base64,\(b.bin.base64EncodedString())")
    ...
    static func writeGLB(_ mesh: Mesh) -> Data {
        ...
        u32(0x4654_6C67)
        u32(2)
        u32(12 + 8 + json.count + 8 + bin.count)  // "glTF", version 2, total length
        u32(json.count)
        u32(0x4E4F_534A)
        d.append(contentsOf: json)  // JSON chunk ("JSON")
        u32(bin.count)
        u32(0x004E_4942)
        d.append(contentsOf: bin)  // BIN chunk ("BIN\0")
        return d
    }
}
