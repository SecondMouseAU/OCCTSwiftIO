...
/// Native Wavefront OBJ reader/writer (geometry only: `v` + `f`).
///
/// Faces of any vertex-ref form (`v`, `v/vt`, `v/vt/vn`, `v//vn`; negative-relative indices) are
/// fan-triangulated. No OCCT.
public enum OBJ {
    ...
    public static func read(data: Data, weldEpsilon: Float = 1e-4) throws -> Mesh {
        ...
        data.withUnsafeBytes { (raw: UnsafeRawBufferPointer) in
            ...
            while i < n {
                ...
                if c0 == UInt8(ascii: "v"), sep {
                    parseVertex(b, s + 1, e, &positions)
                } else if c0 == UInt8(ascii: "f"), sep {
                    parseFace(b, s + 1, e, positions.count, &indices)
                }
            }
        }
    ...
    private static func parseVertex(
        _ b: UnsafeBufferPointer<UInt8>, _ from: Int, _ to: Int, _ out: inout [SIMD3<Float>]
    ) {
        var p = from
        var v = [Float]()
        while p < to, v.count < 3 {
            ...
            let s = p
            while p < to, b[p] != 32, b[p] != 9, b[p] != 13 { p += 1 }
            if p > s,
                let f = Float(
                    String(decoding: UnsafeBufferPointer(rebasing: b[s..<p]), as: UTF8.self))
            {
                v.append(f)
            }
        }
    ...
    private static func parseFace(
        _ b: UnsafeBufferPointer<UInt8>, _ from: Int, _ to: Int, _ vcount: Int,
        _ out: inout [UInt32]
    ) {
        var p = from
        var verts = [UInt32]()
        while p < to {
            ...
            if p > s {
                var e = s
                while e < p, b[e] != UInt8(ascii: "/") { e += 1 }
                if let raw = Int(
                    String(decoding: UnsafeBufferPointer(rebasing: b[s..<e]), as: UTF8.self))
                {
