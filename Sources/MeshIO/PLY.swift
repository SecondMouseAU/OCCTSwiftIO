...
/// Native PLY reader/writer.
///
/// Reads ASCII and binary-little-endian PLY: the `vertex` element's x/y/z and the `face` element's
/// index lists (fan-triangulated). No OCCT.
public enum PLY {

    public static func read(data: Data, weldEpsilon: Float = 1e-4) throws -> Mesh {
        guard !data.isEmpty else { throw MeshError.empty }
        let bytes = [UInt8](data)
        guard bytes.count >= 3, bytes[0] == UInt8(ascii: "p"), bytes[1] == UInt8(ascii: "l"),
            bytes[2] == UInt8(ascii: "y")
        ...
        do {
            ...
            while i < bytes.count {
                ...
                i += 1
                if ch == 10 {
                    let s = String(decoding: line, as: UTF8.self).trimmingCharacters(
                        in: .whitespaces)
                    lines.append(s)
        ...
        for l in lines {
            ...
            switch kw {
            case "format":
                binary = l.contains("binary")
                bigEndian = l.contains("big_endian")
            case "element":
                current = f[1]
                if f[1] == "vertex" {
                    vertexCount = Int(f[2]) ?? 0
                } else if f[1] == "face" {
                    faceCount = Int(f[2]) ?? 0
                }
            case "property":
                if f[1] == "list", current == "face" {
                    ...
                    let (sz, isF) = tsize(f[1])
                    vProps.append(Prop(name: f.last ?? "", size: sz, isFloat: isF))
                }
            default: break
            }
        }
        ...
        if !binary {
            ...
            let offs = vProps.reduce(into: (run: 0, arr: [Int]())) { acc, pr in
                acc.arr.append(acc.run)
                acc.run += pr.size
            }.arr
                    ...
                    return pr.size == 8
                        ? Float(
                            bytes.withUnsafeBytes {
                                $0.loadUnaligned(fromByteOffset: at, as: Double.self)
                            })
                        : bytes.withUnsafeBytes {
                            $0.loadUnaligned(fromByteOffset: at, as: Float.self)
                        }
                }
            ...
            for _ in 0..<vertexCount {
                ...
                positions.append(
                    SIMD3(
                        f(p + offs[xi], vProps[xi]), f(p + offs[yi], vProps[yi]),
                        f(p + offs[zi], vProps[zi])))
                p += stride
            }
            if let fl = faceList {
                for _ in 0..<faceCount {
                    guard p + fl.countSize <= bytes.count else { break }
                    let k = readInt(bytes, p, fl.countSize)
                    p += fl.countSize
                    var idx = [Int]()
                    idx.reserveCapacity(k)
                    for _ in 0..<k {
                        guard p + fl.indexSize <= bytes.count else { break }
                        idx.append(readInt(bytes, p, fl.indexSize))
                        p += fl.indexSize
                    }
        ...
        for face in faces where face.count >= 3 {
            ...
            where face[0] < positions.count && face[k] < positions.count
                && face[k + 1] < positions.count
            {
        ...
        if weldEpsilon > 0 {
            ...
            soup.reserveCapacity(indices.count)
    ...
    private static func readInt(_ b: [UInt8], _ at: Int, _ size: Int) -> Int {
        var v = 0
        for k in 0..<size { v |= Int(b[at + k]) << (8 * k) }
        return v
    }
