...
extension MeshIO {
    ...
    /// Read a **3MF** package into a ``Mesh`` via the ThreeMF package.
    ///
    /// Uses ThreeMF's flattened `LoadedModel` (build items expanded into mesh instances with
    /// accumulated transforms), so multi-object / instanced models are placed correctly.
    static func readThreeMF(data: Data, weldEpsilon: Float) throws -> Mesh {
        ...
        func emit(_ mesh: ThreeMF.Mesh, _ transforms: [Matrix3D]) {
            let vs = mesh.vertices.map { v -> SIMD3<Double> in
                placed(SIMD3(v.x, v.y, v.z), transforms)
            }
    ...
    static func writeThreeMF(_ mesh: Mesh) throws -> Data {
        let verts = mesh.positions.map {
            ThreeMF.Mesh.Vertex(x: Double($0.x), y: Double($0.y), z: Double($0.z))
        }
        ...
        let object = ThreeMF.Object(
            id: 1, content: .mesh(ThreeMF.Mesh(vertices: verts, triangles: tris)))
        let model = ThreeMF.Model(
            resources: [object], build: ThreeMF.Build(items: [ThreeMF.Item(objectID: 1)]))
        let writer = ThreeMF.PackageWriter<Data>()
        writer.model = model
        return try writer.finalize()
    }

    /// Apply a 3MF transform chain (parent→child) to a point.
    ///
    /// Each `Matrix3D` is a 4×3 row-major affine (rows 0–2 = linear part, row 3 = translation);
    /// child-most transform applies first.
    ...
    ///
    /// ThreeMF's loader is async (it unzips + resolves referenced model parts); MeshIO's `load`
    /// API is synchronous.
    private static func runBlocking<T: Sendable>(_ op: @Sendable @escaping () async throws -> T)
        throws -> T
    {
        let box = ResultBox<T>()
