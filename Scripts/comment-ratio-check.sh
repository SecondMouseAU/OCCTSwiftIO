...
#
# Part of the ecosystem's code-style policy trial (OCCTSwiftIO#36, following
# the reference rollout at OCCTSwiftScripts#114); see
...
# (see the proposal's own CurveAdaptors.md finding for what that looks like
# once it's had years to drift): this script surfaces the number, it doesn't
# judge which case it is. Report-only: always exits 0, same as
# check-docs-existence.py's own `--coverage` mode in OCCTSwift/Scripts, and
# for the same reason: failing every file above a threshold on day one, in a
# codebase nobody has swept yet, punishes the report instead of the problem.
#
# Usage:
