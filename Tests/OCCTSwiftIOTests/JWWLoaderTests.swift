...
struct JWWLoaderTests {
    ...
    /// whose in-plane (X) extent matches the drawing.
    ///
    /// (Hermetic synthetic-JWW coverage lives in SwiftJWW).
    @Test func realDrawingExtent() throws {
        let url = URL(
            fileURLWithPath: NSString(string: "~/Documents/Modelling/2DFiles/dd12.jww")
